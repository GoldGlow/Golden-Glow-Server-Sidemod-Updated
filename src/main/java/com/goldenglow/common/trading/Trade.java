...
public class Trade {
    ...
    public TradingOffer getOffer(EntityPlayerMP player){
        for(int i=0;i<offers.length;i++){
            if(offers[i].getPlayer().equals(player)){
                return offers[i];
            }
        }
        return null;
    ...
    public void complete(){
        ...
        /*for (OOItem item : this.getOffers()[0].items) {
            ItemHandlerHelper.giveItemToPlayer(this.getPlayers()[1], itemStack);
        }
        for (OOItem item : this.getOffers()[1].items) {
