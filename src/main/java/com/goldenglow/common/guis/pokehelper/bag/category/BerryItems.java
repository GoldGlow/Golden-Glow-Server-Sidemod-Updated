...
public class BerryItems extends ItemCategoryBase {
    public BerryItems(){
        super("Berries");
    }
    ...
    public String[] getItemNames(EntityPlayerMP player){
        ...
        List<OOItem> berries=data.getBerries();
        String[] berryNames=new String[berries.size()];
        for(int i=0;i<berries.size();i++){
            berryNames[i]=berries.get(i).getQuantity()+"x "+berries.get(i).getDisplayName();
        }
        return berryNames;
    }
    ...
    public CustomGuiWrapper displayItem(EntityPlayerMP player, CustomGuiWrapper gui){
        String texture="pixelmon:textures/items/helditems/";
    ...
    public void withdrawItem(EntityPlayerMP player, OOItem item){
        ...
        item.changeQuantity(-1*amountChanged);
        data.removeBerries(item);
    }

    @Override
    public OOItem getItem(EntityPlayerMP player, int index){
        IPlayerData data=player.getCapability(OOPlayerProvider.OO_DATA, null);
        return data.getBerries().get(index);
    }
}
