...
public class TeachTMMenu implements EssentialsGuis {
    ...
    public void init(EntityPlayerMP player, EntityNPCInterface npc){
        ...
        for(int i=0;i<6;i++){
            if(party.get(i)!=null){
                ...
                if(canLearn){
                    gui.addTexturedButton(i, "Able", coordinates[2], coordinates[3], coordinates[4], coordinates[5], "obscureobsidian:textures/gui/transparent.png");
                    this.addButton(new EssentialsButton(i, new ActionData("TEACH_MOVE", i+":"+this.attackName)));
                }
                else{
                    if(pokemon.getMoveset().hasAttack(new Attack(this.attackName))){
