    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        try {
            if (args.length >= 2) {
                ...
                if (args.length == 5) {
                    BossManager.startBossBattle(server.getPlayerList().getPlayerByUsername(playerName), bossName, Integer.parseInt(args[2]), Integer.parseInt(args[3]), Integer.parseInt(args[4]));
                } else
